...
import { OPENCOPS_MOCK_API_DEFAULT_DELAY } from './client-api.constants';
import { OpencopsMockApiService } from './client-api.service';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(OPENCOPS_MOCK_API_DEFAULT_DELAY);
    const opencopsMockApiService = inject(OpencopsMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = opencopsMockApiService.findHandler(request.method.toUpperCase(), request.url);
